...
CL_Logger *_cl_logger_create(uint16_t ouput_count, const char *name, const char *pattern)
{
	if (g_cl_info == NULL)
	{
		_cl_init(CL_DEFAULT_PATTERN);
		CL_Logger *logger_default = _cl_logger_create(1, "DEFAULT", CL_DEFAULT_PATTERN);
		_cl_logger_output_add(logger_default, stdout, CL_COLOR_ENABLED);
		CL_LOG_ERROR(logger_default, "Could not create logger. The library should be initialized using 'CL_INIT()' before creating a logger.");
		_cl_logger_destroy(logger_default);
		_cl_terminate();
	}
	else
	{
